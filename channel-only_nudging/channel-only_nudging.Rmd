...
This document introduces users of the National Water Model v1.2 to the channel-only and nudging data assimilation features. While nudging data assimilation has been available since NWM v1.0, the channel-only mode is only available with v1.2 so we focus on this version of the code. 

The primary intent here is to show users how to configure and run the model using both channel-only and nudging data assimilation. The source of this document includes all code required to both run the model *and* do the complete analysis (in R) which includes generating all the figures except one. You should receive both versionsPlease contact me (jamesmcc@ucar.edu) if you would  
