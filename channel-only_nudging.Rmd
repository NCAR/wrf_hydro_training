...
```{r, eval=FALSE}
chanOnlyAssimChrtoutFiles <- list.files(path='.', pattern='CHRTOUT', full.names=TRUE)
chanOnlyAssimData <-  ldply(NamedList(chanOnlyAssimChrtoutFiles),
                        GetNcdfFile, variables=c('time'), exclude=TRUE, quiet=TRUE)
chanOnlyAssimData <- as.data.table(chanOnlyAssimData)
chanOnlyAssimData[, `:=`(POSIXct=as.POSIXct(basename(.id), format='%Y%m%d%H', tz='UTC'))]
```
...
setnames(chanOnlyAssimData.gages, 'streamflow', 'assim')
