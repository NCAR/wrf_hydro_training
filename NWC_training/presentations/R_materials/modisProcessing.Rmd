...
output:
  html_document:
    ...
    highlight: tango  ##probably the best, random promtp emphasis is
---
...
# Background
We are using WRF-Hydro to predict streamflow for the Big Thompson USGS gage for the 2013 snowmelt period. We want to obtain MODIS LAI images for our region of interest and insert the processed images into our forcing time series so we can use dynamic LAI in future WRF-Hydro model runs.
...
```{r,eval=FALSE}
devtools::install_github("aubreyd/modis4rwrfhydro")
```
...
gdalPath <- sub("gdalinfo", replacement="", 
                system("which gdalinfo", intern=TRUE))
MODISoptions(localArcPath="~/wrfHydroTestCases/MODIS_ARC", 
             outDirPath="~/wrfHydroTestCases/MODIS_ARC/PROCESSED",
             gdalPath=gdalPath,
             stubbornness="low")
```
...
Here we download the relevant MODIS tiles and resample to the Big Thompson geogrid. The GetMODIS tool calls the MODIS-R runGdal tool to identify the required MODIS tiles to cover the domain, download the appropriate product tiles, mosaic them, and clip/resample (nearest neighbor) to the geogrid extent and resolution. The result will be a directory of processed TIF images in the outDirPath specified in MODISoptions. We specify the geogrid, a start and end date, and the MOD15A2 (FPAR/LAI) MODIS product name.

```{r}
GetMODIS(extent=paste0(dataPath, '/DOMAIN/geo_em.nc'), prodName="MOD15A2",  
         outDir="BIGT_LAI", begin="2013.03.01", end="2013.07.31",
...
```{r}
lai.b <- ConvertRS2Stack(paste0(options("MODIS_outDirPath"), '/BIGT_LAI'), "*Lai_1km.tif$", 
...
```{r}
ConvertStack2NC(lai.b, outFile=paste0(dataPath, '/DOMAIN/BIGT_LAI_SUM13.nc'), 
...
lai.b.sm1 <- SmoothStack(lai.b, 
                         outDirPath=paste0(options("MODIS_outDirPath"), '/BIGT_LAI_SMOOTH1'), 
...
lai.b.sm2 <- SmoothStack(lai.b, 
                         outDirPath=paste0(options("MODIS_outDirPath"), '/BIGT_LAI_SMOOTH2'), 
